...
GO
IF EXISTS (SELECT * FROM IFOSUP_Trans.sys.tables WHERE name = 'Compte_epargne')
DROP TABLE Compte_epargne
GO
CREATE TABLE Compte_epargne
(
AccountNum Money NOT NULL,   -- on aurait aussi travailler avec INT mais avec Money, on peut préciser la devise !
Amount Money NOT NULL
)
SELECT * FROM Compte_epargne
GO

IF EXISTS (SELECT * FROM IFOSUP_Trans.sys.tables WHERE name = 'Compte_courant ')
DROP TABLE Compte_courant 
GO
CREATE TABLE Compte_courant  
(
AccountNum Money NOT NULL,
Amount Money NOT NULL
)
GO
